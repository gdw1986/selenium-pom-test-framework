...
TC_Login_003 Login With Wrong Username
    ...
    Wait For Elements State    \#login-error    visible    5s
...
TC_Login_004 Login With Wrong Password
    ...
    Login With Credentials    ${LOGIN_USERNAME}    wrongpassword
    Wait For Elements State    \#login-error    visible    5s
    ${error_text}=    Get Text    \#login-error
    Should Contain    ${error_text}    用户名或密码错误
...
TC_Login_008 Clear Input Fields
    ...
    Clear Text    \#username
    Clear Text    \#password
    ${u_val}=    Get Property    \#username    value
    ${p_val}=    Get Property    \#password    value
